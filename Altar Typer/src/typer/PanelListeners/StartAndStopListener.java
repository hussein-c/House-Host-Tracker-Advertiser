...
import typer.Panels.MenuBarPanel;
...
public class StartAndStopListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "Start Typing":
			...
			if (errorFlag == false) { //If the advertisements are not empty, start the typer
				JOptionPane.showMessageDialog(Settings.frame, "Once you press okay, this program will minimize.\nYou have 5 seconds to click inside the OSRS/RS3 client.");
				Settings.frame.setState(Frame.ICONIFIED);
	...
	private void emptyAdvertisementValidation() { //Checks whether the advertisements are empty, and if so, it stops the user from starting the typer if they enabled the advertisement check box
		String onlyCommunityAdvertisements = Settings.communityMessages.trim();
		String onlyBoneJobAdvertisements = Settings.boneJobMessages.trim();
		errorFlag = false;
		if (CentralPanel.onlyCommunityAdvertisements.isSelected() && CentralPanel.onlyBoneJobAdvertisements.isSelected()) {
			if (onlyCommunityAdvertisements.equals("") && onlyBoneJobAdvertisements.equals("")) {
				errorFlag = true;
				JOptionPane.showMessageDialog(Settings.frame, "Please add some community or bone job advertisements or uncheck their respective checkboxes.");
				return;
			}
		}
			...
			if (onlyCommunityAdvertisements.equals("")) {
				errorFlag = true;
				JOptionPane.showMessageDialog(Settings.frame, "Please add some community advertisements or uncheck its checkbox.");
				return;
			}
		}
			...
			if (onlyBoneJobAdvertisements.equals("")) {
				errorFlag = true;
				JOptionPane.showMessageDialog(Settings.frame, "Please add some bone job advertisements or uncheck its checkbox.");
				return;
			}
		}
	}
