...
public class Config {
	...
	public static void replaceConfigFileDialog() { //User confirmation dialogue for when the configuration file is created or replaced
		if (configFileDeletion == true) {
			JOptionPane.showMessageDialog(Settings.frame, "Config file replaced:\n" + configFile + "\nIts configuration has been applied automatically.", "Config File", JOptionPane.INFORMATION_MESSAGE);
		}
		else {
			JOptionPane.showMessageDialog(Settings.frame, "Config file created:\n" + configFile + "\nIts configuration has been applied automatically.", "Config File", JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	public static void updateConfigFile() { //Updates the configuration file with new data
		String[] linesToWriteToConfigFile = {"#TYPING_SPEED" + "\n",
				...
				"TYPING_SPEED_MIN=" + Settings.adjustTyperSettings.getTypeSpeedDelayMin() + "\n",
				"TYPING_SPEED_MAX=" + Settings.adjustTyperSettings.getTypeSpeedDelayMax() + "\n",
				"\n",
				"#ADVERTISING_LINES_DELAY" + "\n",
				"[This must be entered in seconds]" + "\n",
				"LINES_DELAY_MIN=" + Settings.adjustTyperSettings.getDelayTimeMin() / 1000 + "\n",
				"LINES_DELAY_MAX=" + Settings.adjustTyperSettings.getDelayTimeMax() / 1000 + "\n",
				"\n",
				"#STOP_TIMER" + "\n",
				"[This must be entered in hours (1 hour or more)]" + "\n",
				"STOP_TIME=" + Settings.adjustTyperSettings.getStopTime() / 3600 + "\n",
				"\n",
				"#HOST_COUNT" + "\n",
				"[This is the number of host lines that occur before an advertisement line (1 host line or more)]" + "\n",
				"HOST_COUNT=" + Settings.adjustTyperSettings.getHostCount() + "\n",
				"\n",
				...
				"[8 = OSRS - 465 - Yanille]" + "\n", 
				"[9 = OSRS - 512 - Yanille]" + "\n", 
				"SERVER_DROPDOWN_MENU=" + Settings.mainPageSettings.getServerDropdownMenuIndex() + "\n",
				"\n",
				"#PIN_TO_TOP" + "\n",
				"[This allows you to keep this program on top of other programs. 0 = unchecked, 1 = checked]" + "\n",
				"PIN_TO_TOP=" + Settings.mainPageSettings.getPinToTop() + "\n",
				"\n",
				"#ONLY_VERIFIED_HOSTS" + "\n",
				"[This allows you to only advertise verified hosts. 0 = unchecked, 1 = checked]" + "\n",
				"ONLY_VERIFIED_HOSTS=" + Settings.mainPageSettings.getOnlyVerifiedHosts() + "\n",
				"\n",
				"#ADVERTISE_TRACKER_HOSTS" + "\n",
				"[This allows you to advertise active hosts listed on the tracker. 0 = unchecked, 1 = checked]" + "\n",
				"ADVERTISE_TRACKER_HOSTS=" + Settings.mainPageSettings.getAdvertiseTrackerHosts() + "\n",
				"\n",
				"#ADVERTISE_COMMUNITY_LINES" + "\n",
				"[This allows you to advertise community advertisements. 0 = unchecked, 1 = checked]" + "\n",
				"ADVERTISE_COMMUNITY_LINES=" + Settings.mainPageSettings.getAdvertiseCommunityLines() + "\n",
				"\n",
				"#ADVERTISE_BONE_JOB_LINES" + "\n",
				"[This allows you to advertise bone job advertisements. 0 = unchecked, 1 = checked]" + "\n",
				"ADVERTISE_BONE_JOB_LINES=" + Settings.mainPageSettings.getAdvertiseBoneJobLines() + "\n",
				"\n",
				"#COLOURS" + "\n",
				"[This allows you to choose the colour(s) that you wish to use while typing. 0 = unchecked, 1 = checked]" + "\n",
				"Yellow=" + Settings.colourAndEffectSettings.getYellow() + "\n",
				"Purple=" + Settings.colourAndEffectSettings.getPurple() + "\n",
				"Red=" + Settings.colourAndEffectSettings.getRed() + "\n",
				"Green=" + Settings.colourAndEffectSettings.getGreen() + "\n",
				"White=" + Settings.colourAndEffectSettings.getWhite() + "\n",
				"Cyan=" + Settings.colourAndEffectSettings.getCyan() + "\n",
				"Flash1=" + Settings.colourAndEffectSettings.getFlash1() + "\n",
				"Flash2=" + Settings.colourAndEffectSettings.getFlash2() + "\n",
				"Flash3=" + Settings.colourAndEffectSettings.getFlash2() + "\n",
				"Glow1=" + Settings.colourAndEffectSettings.getGlow1() + "\n",
				"Glow2=" + Settings.colourAndEffectSettings.getGlow2() + "\n",
				"Glow3=" + Settings.colourAndEffectSettings.getGlow3() + "\n",
				"\n",
				"#EFFECTS" + "\n",
				"[This allows you to choose the effect(s) that you wish to use while typing. 0 = unchecked, 1 = checked]" + "\n",
				"Wave=" + Settings.colourAndEffectSettings.getWave() + "\n",
				"Wave2=" + Settings.colourAndEffectSettings.getWave2() + "\n",
				"Scroll=" + Settings.colourAndEffectSettings.getScroll() + "\n",
				"Slide=" + Settings.colourAndEffectSettings.getSlide() + "\n",
				"Shake=" + Settings.colourAndEffectSettings.getShake() + "\n"};
	...
	public static void writeToConfigFile(String[] lines, String[] noHostSpamLine, String[] communitySpamLines, String[] boneJobSpamLines) { 
		...
		try {
			...
			JOptionPane.showMessageDialog(Settings.frame, "Oops! Something went wrong with writing to the config file. Please delete the config file located at:\n" + configFile, "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public static String[] readFromConfigFile() { //Reads the advertisements and settings data from the configuration file
		...
		try {
			...
			JOptionPane.showMessageDialog(Settings.frame, "Oops! Something went wrong with reading the config file. Please delete the config file located at:\n" + configFile, "Error", JOptionPane.ERROR_MESSAGE);
		}
	...
	private static void backupConfigFile() { //Backs up the configuration file when it is corrupt or missing important information
		...
		try {
			...
			JOptionPane.showMessageDialog(Settings.frame, configFile + " could not be located.", "Config File", JOptionPane.ERROR_MESSAGE);
			return;
		}
		String configFileContents = "";
		while (scanner.hasNextLine()) {
		...
		if (selectedOption == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				JOptionPane.showMessageDialog(Settings.frame, "Oops! Something went wrong with writing to the config backup file. Please delete the config backup file located at:\n" + backupConfigFile, "Error", JOptionPane.ERROR_MESSAGE);
			}
	...
	public static void configErrorMessage() { //Error message that displays to the user when the data in the configuration file is missing or invalid
		Settings.configErrorMessage = "The config file was unable to be read. Its settings have either a non-numeric value or been deleted.\nAs a result, the config file will be replaced. You may wish to back up the current config file.";
		Object[] options = {"Continue with backup", "Continue without backup"};
		int dialogOption = JOptionPane.showOptionDialog(Settings.frame, Settings.configErrorMessage,
	...
	public static void newConfigFile() { //Creates a new configuration file if the user wishes to do so
		replaceConfigFile();
		Settings.mainPageSettings.setServerDropdownMenuIndex(CentralPanel.serverType.getSelectedIndex());
	...
	public static void openConfigFile() { //Opens the existing configuration file if the user wishes to do so
		if (configFile.exists()) {
			try {
				...
				JOptionPane.showMessageDialog(Settings.frame, "Oops! Something went wrong with opening the config file. Please try again.", "Config File", JOptionPane.ERROR_MESSAGE);
			}
		}
		else {
			JOptionPane.showMessageDialog(Settings.frame, configFile + " could not be located.", "Config File", JOptionPane.ERROR_MESSAGE);
	...
	public static void applyChanges() { //Applies any changes made by the user to the configuration file and settings
		if (configFile.exists()) {
			Settings.initialiseSettings();
			CentralPanel.serverType.setSelectedIndex(Settings.mainPageSettings.getServerDropdownMenuIndex());
			updateConfigFile();
			setFrameOnTop();
			MainCheckboxUpdater.updateMainCheckBoxes();
			HostUpdaterThread.listOnlyVerifiedHosts();
			if (Settings.errorFlag == false) {
				JOptionPane.showMessageDialog(Settings.frame, "Any changes you made to your config file have now been applied.", "Config File", JOptionPane.INFORMATION_MESSAGE);
			}
		}
		else {
			JOptionPane.showMessageDialog(Settings.frame, configFile + " could not be located.", "Config File", JOptionPane.ERROR_MESSAGE);
	...
	public static void openWebpage(String url) { //Opens the web page URL depending on which button the user clicked on the menu bar
		...
		if (desktop != null && desktop.isSupported(Desktop.Action.BROWSE)) {
			try {
				...
				JOptionPane.showMessageDialog(Settings.frame, "Oops! Something went wrong with opening the Web page. Please try again.", "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
