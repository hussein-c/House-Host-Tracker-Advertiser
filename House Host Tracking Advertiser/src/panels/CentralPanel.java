...
import actionListeners.ColoursAndEffectsListener;
import actionListeners.ConfigsListener;
...
public class CentralPanel extends JPanel {
	...
	private void serverType() { //Server type drop-down menu
		...
		serverType.setSelectedIndex(StartupSettings.mainPageSettings.getServerDropdownMenuIndex());
		...
		serverType.addActionListener(e -> {
			...
			StartupSettings.mainPageSettings.setServerDropdownMenuIndex(serverType.getSelectedIndex());
			ServerTypeMenuListener.setCommunityAdvertisementsText();
	...
	private void blackList() { //Black List
		...
		blackList.addActionListener(e -> {
			...
			int listSize = 1;
			for (String name : Website.blackList) {
			...
			JOptionPane.showMessageDialog(StartupSettings.frame, message, "Black List", JOptionPane.INFORMATION_MESSAGE);
		});
		add(blackList);
	}
